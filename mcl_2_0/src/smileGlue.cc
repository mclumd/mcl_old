...
int smileGlue::smileIndex2configIndex(int smileI,
  ...
  for (int smI=0; smI < depsAsCreated.size(); smI++) {
	...
	if (depsAsCreated[smI] == hgD[hgI])
      ...
      if (q2hg == -1) {
	umbc::exceptions::signal_exception("error mapping smile index to config index: usually indicates smile dependency could not be found in home-grown network dependency list: "+depsAsCreated[smI]);
      }
...
smileNodeGlue::smileNodeGlue(string name,mclNode* node) :
  ...
  if (getFrameGlue() == NULL) {
    umbc::exceptions::signal_exception("smile frame glue is NULL [cannot create smileNodeGlue before smileFrameGlue has been attached to mclFrame]");
  }
