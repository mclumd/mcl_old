...
bool mclMA::initializeMCL(string key,int Hz) {
  ...
  umbc::exceptions::set_exception_handlers();
...
void mclMA::expectationGroupComplete(string key,egkType eg_key,float *sv,int svs) {
  umbc::uLog::annotate(MCLA_WARNING,"SVS-based expectationGroupComplete() is deprecated.");
  mcl* this_mcl = get_mcl(key);
  this_mcl->setSV(sv,svs);
...
void mclMA::suggestionImplemented(string key,resRefType referent) {
  ...
  if (frame == NULL) {
    umbc::exceptions::signal_exception("suggestionImplemented called with NULL referent.");
  }
  else {
    // it might be smart to do something else here, like
    // record the time at which the suggestion was accepted (for durative
    // & diagnostic purposes)
    umbc::uLog::annotate(MCLA_MSG,"[mcl/hostAPI]:: host has indicated that response to "+ frame->entityName() +" has been triggered.");
...
void mclMA::suggestionFailed(string key,resRefType referent) {
  ...
  mclFrame *frame = this_mcl->referent2frame(referent);
  if (frame == NULL) {
    umbc::exceptions::signal_exception("suggestionFailed called with NULL referent.");
  }
  else {
...
void mclMA::suggestionIgnored(string key,resRefType referent) {
  ...
  mclFrame *frame = this_mcl->referent2frame(referent);
  if (frame == NULL) {
    umbc::exceptions::signal_exception("suggestionIgnored called with NULL referent.");
  }
  else {
...
void mclMA::provideFeedback(string key,bool feedback, resRefType referent) {
  ...
  if (frame == NULL) {
    ...
    umbc::exceptions::signal_exception("provideFeedback called with NULL or NON-FRAME referent.");
  }
  else {
...
bool mclMA::writeInitializedCPTs(string key) {
  ...
  if (m == NULL) {
    ...
    mclFrame    *f = new mclFrame(m,NULL,RESREF_NO_REFERENCE);
