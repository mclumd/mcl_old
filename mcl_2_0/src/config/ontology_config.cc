...
string ontology_configuration::determine_paths() {
  ...
  if ((pathToMCLCore != NULL) && 
    ...
    // otherwise use CWD
    umbc::uLog::annotate(MCLA_WARNING,"[mcl/oc]:: could not confirm user preference for write path using MCL_CORE_PATH or HOME. Using CWD as root for writing configurations.");
    write_path = "./";
  }

  umbc::uLog::annotate(MCLA_MSG,"[mcl/oc]:: config write path is '"+write_path+"'");
  ...
  umbc::uLog::annotate(MCLA_WARNING,"[mcl/oc]:: could not find config in MCL_CORE_PATH or HOME. Ontologies will not be configured by configManager.\n");
  return "";
}
