...
bool mcl_AD_InteractiveResponse::interpretFeedback(bool feedback) {
  for (vector<string>::iterator pi = positiveList.begin();
    ...
    if (pnode == NULL) {
      ...
      umbc::uLog::annotate(MCLA_MSG,"[mcl/oNode]::asserting A/D+ dependency "+*pi+" set to false.");
  ...
  for (vector<string>::iterator ni = negativeList.begin();
    ...
    if (nnode == NULL)
      ...
      umbc::uLog::annotate(MCLA_MSG,"[mcl/oNode]::asserting A/D- dependency "+*ni+" set to false.");
      nnode->set_evidence(false);
    }
    else {
      umbc::uLog::annotate(MCLA_MSG,"[mcl/oNode]::asserting A/D- dependency "+*ni+" set to true.");
...
string mclConcreteResponse::countsAsString() {
  ...
  sprintf(buff,"(fail=%d,succ=%d,ignore=%d,abort=%d)",
	  failedAttempts,successfulAttempts,ignores,abortedAttempts);
  return (string)buff;
}
