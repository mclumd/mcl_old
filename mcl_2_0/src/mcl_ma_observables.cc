...
void mclMA::observables::add_obs_legalval_self(string mclkey,string name,
  ...
  tmcl->add_obs_legalval(name,lval);
}

void mclMA::observables::add_obs_legalval(string mclkey,
					  string objname,string obsname,
					  double lval) {
  mcl* tmcl = get_or_create(mclkey);
  tmcl->add_obs_legalval(objname,obsname,lval);
}
...
void mclMA::observables::set_obs_legalrange_self(string mclkey,
  ...
  mcl* tmcl = get_or_create(mclkey);
  tmcl->set_obs_legalrange(obsname,low,high);
}

void mclMA::observables::set_obs_legalrange(string mclkey,
					    string objname,string obsname,
					    double low,double high) {
  mcl* tmcl = get_or_create(mclkey);
  tmcl->set_obs_legalrange(objname,obsname,low,high);
}
