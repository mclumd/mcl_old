...
hgNodeGlue::hgNodeGlue(string name, mclNode* node) :
  nodeGlue(name,node),hardVal(0.0),hardSet(false),cp_table(NULL) {
...
bool hgNodeGlue::initPriors(vector<string>& d,vector<double>& v) {
  if (mclSettings::debug) {
    *mclLogger::mclLog << "[mcl/hgGlue]::initializing priors: " 
...
void hgNodeGlue::ensure_cpt() {
  if (cp_table == NULL) {
    ...
    *mclLogger::mclLog << "[mcl/hgGlue]:: warning: cp_table is inconsistent across ensure_cpt calls and is being rebuilt." << endl;
    delete cp_table;
    cp_table = new CPT(myNode()->inLinkL());
  }
}
