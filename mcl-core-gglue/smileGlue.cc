...
int smileGlue::smileIndex2configIndex(int smileI,
  ...
  for (int smI=0; smI < depsAsCreated.size(); smI++) {
    // if the bit is set in the smileI, then we need to add to the rv
    if ((smileI & (1 << smI)) != 0) {
      // compute where depAsCreated[smI] is in hgD
      int q2hg=-1;
      for (int hgI=0; hgI < hgD.size(); hgI++) {
	if (depsAsCreated[smI] == hgD[hgI])
	  { q2hg = hgI; break; }
      }
      // set the bit in the return vector accordingly
      if (q2hg == -1) {
	signalMCLException("error mapping smile index to config index: usually indicates smile dependency could not be found in home-grown network dependency list: "+depsAsCreated[smI]);
      }
...
smileNodeGlue::smileNodeGlue(string name,mclNode* node) :
  ...
  if (getFrameGlue() == NULL) {
    signalMCLException("smile frame glue is NULL [cannot create smileNodeGlue before smileFrameGlue has been attached to mclFrame]");
  }
...
void smileNodeGlue::mpd(double* pd) {  
  mclLogger::annotate(MCLA_WARNING,"[SMILEG]: mpd not working...");
}
...
bool smileFrameGlue::build() {
  ...
  // model...
  mclLogger::annotate(MCLA_VRB,"[SMILEG]: creating SMILE nodes...");
